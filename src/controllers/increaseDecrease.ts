...
export async function increaseDecrease(req: Request, res: Response): Promise<Response> {
  try {
    ...
    const url = `${origin}/api/mod/ecommerce/order/shoppingcartAction/${itemId}`;
    const cookie = `cartId=${cartId}; eucookie=1; PHPSESSID=${phpsessid}`;
    let data: unknown = await getApi(url, cartId, uniqueDeviceId, cookie, action);
    if (typeof data === "string") { try { data = JSON.parse(data); } catch {} }
    return res.status(200).json({ success: true, cartId, uniqueDeviceId, resource: action, data });
  } catch (err) {
