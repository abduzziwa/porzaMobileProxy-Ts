...
export async function resetFilters(req: Request, res: Response): Promise<Response> {
  try {
    const origin = process.env.END_POINT!;
    const { phpsessid, cartId, uniqueDeviceId, catId } = req.body as Record<string, string>;
    ...
    const url = `${origin}/api/mod/ecommerce/categorie/resetFilters`;
    const resource = { catid: catId };
