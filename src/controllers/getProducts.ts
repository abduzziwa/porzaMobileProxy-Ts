...
export async function getProducts(req: Request, res: Response): Promise<Response> {
  ...
  try {
    ...
    const productData = await API("/products/data", "POST", { products: productIds, language, options: { oenumbers: false, images: true, usageNumbers: false, measurements: true, package_measurements: false, stock: true, categories: true, brand: true }, page: 1, limit: 30 });
    const formatted = Object.values((productData?.products ?? {}) as Record<string, RawProduct>)
      .map(formatProduct)
      .filter((p: any) => {
        // Never send products with zero purchasable stock
        const total = Number(p?.stock?.total_available ?? 0);
        const external = Number(p?.stock?.external_available ?? 0);
        const allowed = p?.stock?.order_allowed === "1";
        return allowed && (total > 0 || external > 0);
      });
